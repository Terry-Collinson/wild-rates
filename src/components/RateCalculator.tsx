  ...
  const sanctuarySuites = useMemo(() => {
    ...
    if (ingestedRoomTypes && ingestedRoomTypes.length > 0) {
      return ingestedRoomTypes.map(rt => {
        ...
        // Resolve photographs priority list: Override URL -> Synced Local Images -> Raw API Google CDN Images
        ...
        if (localImagesList.length > 0) {
          // Prioritize newly downloaded genuine local JPEG files!
                ...
                <Card key={idx} className="glass-card border-white/5 bg-black/40 overflow-hidden flex flex-col md:flex-row group hover:border-primary/40 transition-all shadow-2xl relative items-stretch">
                  ...
                  <CardContent className="flex-1 p-10 flex flex-col justify-between">
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between mt-10 pt-8 border-t border-white/10 gap-6">
                      ...
                      {/* Checkout Integration Preview Dialog */}
                                ...
                                <div className="bg-amber-500/10 border-b border-amber-500/20 p-6">
                                  <DialogHeader>
                                    <DialogTitle className="text-xl font-headline italic flex items-center gap-2 text-white">
                                      ...
                                      {isProfitroom ? 'Profitroom CRS Integration — Session Handoff' : 'NightsBridge API Integration — Placeholder'}
                                    </DialogTitle>
                                    <DialogDescription className="text-amber-400/80 text-xs font-medium leading-relaxed mt-1">
                                      {isProfitroom 
                                        ? 'Integration active. A pre-discounted secure session token will be generated by our server and used to hand off checkout directly to Profitroom\'s Step 3 payment gateway container.'
                                        : '⚠️ Integration pending. This shows the outbound request we will send and the expected response shape from NightsBridge once we have their API specification.'
                                      }
                                    </DialogDescription>
                                  </DialogHeader>
                                </div>
                                ...
                                <div className="p-6 space-y-6">
                                  ...
                                  <div className="space-y-2">
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary flex items-center gap-2">
                                      <ArrowRight className="w-3 h-3" /> Outbound — {isProfitroom ? 'POST /api/booking/checkout' : 'POST /api/nightsbridge/book'}
                                    </p>
                                  ...
                                  <div className="space-y-2">
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50 flex items-center gap-2">
                                      <ChevronRight className="w-3 h-3" /> {isProfitroom ? 'Expected Handoff Payload' : 'Expected Response — shape unknown, placeholder below'}
                                    </p>
                                    <pre className="p-4 rounded-xl bg-black/60 border border-white/10 text-[11px] font-mono text-white/40 leading-relaxed overflow-x-auto">
                                      {isProfitroom ? JSON.stringify({
                                        ...
                                        targetUrl: `https://booking.profitroom.com/hotel/v3/${hotelId}/step3/`,
                                        method: "POST",
                                        payload: {
                                          hotelId: hotelId,
                                          checkIn: format(startDate, 'yyyy-MM-dd'),
                                          ...
                                          rateCode: "WILDLIFEHERO-CUG",
                                          discountCode: "WILD5",
                                          token: "[Cryptographic_Auth_Session_ID]"
                                        }
                                      }, null, 2) : JSON.stringify({
                                  ...
                                  <div className="p-4 bg-white/5 rounded-xl border border-white/10 text-[11px] text-white/60 leading-relaxed space-y-1">
                                    ...
                                    {isProfitroom ? (
                                      <>
                                        ...
                                        <p>2. A secure session handoff token is generated by our server to sign checkout rates.</p>
                                        <p>3. A dynamic POST form submits your authenticated details securely to Profitroom.</p>
                                        <p>4. You are redirected to Profitroom's Step 3 Checkout payment container.</p>
                                      </>
                                    ) : (
                                      <>
                                        <p>1. A <span className="text-primary font-bold">Firestore quote document</span> is created with status <code className="text-primary">Pending</code>.</p>
